...
#    def batch_upsert(self, ids: list[str], vectors: list[list[float]], payloads: list[dict]) -> None:
#        from qdrant_client.models import PointStruct

#        points = [
#            PointStruct(
#                id=hashlib.md5(i.encode()).hexdigest()[:16],
#                vector=v,
#                payload=p,
#            )
...
#    """Download a PDF from arXiv given an ID (e.g. '2401.00001' or '2401.00001v2')..

#    Returns the local filepath on success, None on failure.
#    """
#     Strip any URL prefix the user might paste
...
#        logger.error("arXiv download failed for %s: %s", arxiv_id, exc)
#        return None
def download_arxiv_pdf(arxiv_id: str, dest_dir: str) -> Optional[str]:
...
#    """Fetch title, authors, abstract from the arXiv API (no external deps)."""
...
def page_ingest() -> None:
    ...
    tab_arxiv, tab_upload = st.tabs(["arXiv Paper", "Upload PDF"])
